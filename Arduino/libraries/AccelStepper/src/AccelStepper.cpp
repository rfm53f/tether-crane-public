...
unsigned long AccelStepper::computeNewSpeed()
{
  ...
  if (distanceTo > 0)
  {
    ...
    else if (_n < 0)
    {
      ...
      if ((stepsToStop < distanceTo) && _direction == DIRECTION_CW && _speed < _maxSpeed)
  ...
  else if (distanceTo < 0)
  {
    ...
    {
	    // Currently decelerating, need to accel again?
	    if ((stepsToStop < -distanceTo) && _direction == DIRECTION_CCW && -_speed < _maxSpeed)
        _n = -_n; // Start accceleration
    }
  }
...
boolean AccelStepper::runSpeedAccel(bool dir) {
  long bigNum = 1000000;
  
  move(dir ? bigNum : -bigNum);
  return run();
}
